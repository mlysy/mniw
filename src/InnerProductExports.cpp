...
Eigen::MatrixXd CrossProdVXY(Eigen::MatrixXd X, Eigen::MatrixXd Y,
  ...
  for(int ii=0; ii<N; ii++) {
    // single X'V or VY
    if(singleV & singleX) {
      Y.block(0,ii*r,q,r).noalias() = Xt * Y.block(0,ii*r*(!singleY),p,r);
    }
    else {
      if(singleV & singleY) {
	Y.block(0,ii*r,q,r).noalias() = X.block(0,ii*q*(!singleX),p,q) * VY;
      }
    }
	...
	lltp.compute(V.block(0,ii*p,p,p));
      ...
      VX = lltp.solve(Y.block(0,ii*r*(!singleY),p,r));
    }
    if(singleX) {
      Y.block(0,ii*r,q,r).noalias() = Xt * VX;
    }
    else {
      Y.block(0,ii*r,q,r).noalias() = X.block(0,ii*q,p,q).adjoint() * VX;
    }
  }
  return(Y);
}
